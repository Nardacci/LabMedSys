...
document.querySelector('#signup-form').onsubmit=async e=>{e.preventDefault();const f=e.currentTarget,m=document.querySelector('#form-message'),b=document.querySelector('#signup-button'),full_name=f.fullName.value.trim(),company_name=f.companyName.value.trim(),workspace_slug=slugify(company_name);msg(m,'');if(f.password.value!==f.confirmPassword.value)return msg(m,'Passwords do not match.');b.disabled=true;b.textContent='Creating...';try{const {data,error}=await supabase.auth.signUp({email:f.email.value.trim(),password:f.password.value,options:{emailRedirectTo:`${window.location.origin}${window.location.pathname}?confirmed=1`,data:{full_name,company_name,workspace_slug}}});if(error)throw error;if(data.session) await supabase.auth.signOut();
    f.reset();
    location.hash='check-email';}catch(err){msg(m,err.message||'Unable to create your workspace.')}finally{b.disabled=false;b.textContent='Create workspace →'}};
}


function renderCheckEmail(){
  app.innerHTML=`<main class="welcome-page"><section class="welcome-panel check-email-panel"><div class="welcome-logo"><div class="brand-mark">L</div><span>Lab<span>Med</span>Sys</span></div><div class="welcome-content"><div class="welcome-check">✉</div><div class="eyebrow">WELCOME TO LABMEDSYS</div><h1>Welcome aboard! 👋</h1><p>One more step to get started.</p><p class="welcome-description">We've sent a confirmation link to your email. Please confirm your account to activate your secure pharmaceutical workspace.</p><div class="check-email-note"><strong>Didn't receive the email?</strong><span>Check your spam folder or return to sign in and request a new confirmation link.</span></div><a class="welcome-login-link" href="#login">Back to Sign in →</a></div><div class="auth-version">LabMedSys · SaaS Foundation · Development</div></section></main>`;
}

function renderWelcome(user,workspace){
const name=user.user_metadata?.full_name||user.email;
const firstName=name.split(' ')[0];
const company=workspace?.workspaces?.name||user.user_metadata?.company_name||'your company';
const initial=firstName[0].toUpperCase();
app.innerHTML=`<main class="welcome-page"><section class="welcome-panel"><div class="welcome-logo"><div class="brand-mark">L</div><span>Lab<span>Med</span>Sys</span></div><div class="welcome-content"><div class="welcome-check">✓</div><div class="eyebrow">WORKSPACE CREATED</div><h1>Welcome, ${firstName}.</h1><p>Your workspace for <strong>${company}</strong> is ready to go.</p><p class="welcome-description">You now have a secure foundation for building and managing your pharmaceutical operations.</p><div class="welcome-workspace"><div class="workspace-avatar">${initial}</div><div><span>YOUR WORKSPACE</span><strong>${company}</strong></div></div><button id="enter-workspace">Enter workspace →</button><button id="welcome-logout" class="welcome-secondary">Sign out</button></div><div class="auth-version">LabMedSys · SaaS Foundation · Development</div></section></main>`;
document.querySelector('#enter-workspace').onclick=()=>location.hash='dashboard';
document.querySelector('#welcome-logout').onclick=async()=>{await supabase.auth.signOut();location.hash='login'};
}
...
if(location.hash==='#check-email')return renderCheckEmail();
...
if(location.hash==='#dashboard' || location.hash==='#welcome'){
  ...
  try{
    ...
    if(location.hash==='#welcome') return renderWelcome(session.user,workspace);
    return renderDashboard(session.user,workspace);
  }catch(e){console.error(e);location.hash='login';return}
}
if(session){location.hash='welcome';return}
renderLogin();
}
